...
public class GeneratedAssembly
{
    ...
    /// <summary>
    ///     True when this assembly is compiled on its own, without the other files of its
    ///     <see cref="ICodeFileCollection" /> — which is what <c>codegen test</c>
    ///     (<see cref="DynamicCodeBuilder.TryBuildAndCompileAll" />) does, one in-memory assembly per
    ///     <see cref="ICodeFile" />. A type generated by a sibling file cannot be referenced from here,
    ///     so <see cref="AotRootsCompanionExtensions.AddAotRoots(GeneratedAssembly, string, IEnumerable{AttributeArg})" />
    ///     leaves out the <see cref="AttributeArg.TypeNamed" /> roots that do not resolve to a type of
    ///     this assembly. Stamped before <see cref="ICodeFile.AssembleTypes" />, like
    ///     <see cref="TargetLanguage" />.
    /// </summary>
    /// <remarks>
    ///     Never set on a path whose code is written to disk: the written file is compiled together
    ///     with its siblings, and it is the one the trimmer sees. See wolverine#4486.
    /// </remarks>
    public bool CompiledInIsolation { get; set; }
