...
public static class CodegenScenario
{
        ...
        GenerationRules? rules = null)
    {
        if (typeof(TObject).GetMethods().Length != 1)
        {
            throw new ArgumentOutOfRangeException(nameof(TObject),
                "The supplied base type or interface can only have exactly one declared method");
        }
    ...
    public static CodegenResult<IAction<TResult, T1>> ForBuilds<TResult, T1>(
        Action<GeneratedType, GeneratedMethod> configuration, GenerationRules? rules = null)
    {
        return ForBaseOf<IAction<TResult, T1>>(configuration, rules);
    }

    public static CodegenResult<IAction<TResult, T1, T2>> ForBuilds<TResult, T1, T2>(
        Action<GeneratedType, GeneratedMethod> configuration, GenerationRules? rules = null)
    {
        return ForBaseOf<IAction<TResult, T1, T2>>(configuration, rules);
    }

    public static CodegenResult<IAction<TResult, T1, T2, T3>> ForBuilds<TResult, T1, T2, T3>(
        Action<GeneratedType, GeneratedMethod> configuration, GenerationRules? rules = null)
    {
    ...
    public static CodegenResult<IAction<TResult, T1, T2>> ForBuilds<TResult, T1, T2>(
        Action<GeneratedMethod> configuration, GenerationRules? rules = null)
    {
