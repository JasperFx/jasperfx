...
public sealed class AggregateEvolverGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // The marker (jasperfx#887): emitted whether or not this compilation has a single candidate,
        // which is the entire point — an assembly the analyzer processed and an assembly it never saw
        // were otherwise indistinguishable at runtime, so "No source-generated dispatcher found" had to
        // describe a csproj problem and a code-shape problem at once.
        //
        // Not RegisterPostInitializationOutput, because post-initialization cannot see the compilation:
        // this package is analyzer-only and declares no dependency, so a project can carry the analyzer
        // without referencing JasperFx.Events at all, and an unconditional `[assembly: ...]` would turn
        // that build from "compiles, silently ungenerated" into CS0246. Selecting the availability of the
        // attribute down to a bool keeps this incremental — the output re-runs only when that bool flips,
        // never on an edit — while still being independent of the candidate pipelines.
        var markerAvailable = context.CompilationProvider.Select(static (compilation, _) =>
            compilation.GetTypeByMetadataName(MarkerAttributeMetadataName) != null);

        context.RegisterSourceOutput(markerAvailable, static (spc, available) =>
        {
            if (!available) return;
    ...
    /// netstandard2.0 analyzer cannot reference. The attribute allows multiple applications on purpose,
    /// so a compilation that loads this analyzer twice (jasperfx#462) gets two markers rather than CS0579.
    /// </summary>
    private const string MarkerAttributeMetadataName =
        "JasperFx.Events.Aggregation.JasperFxSourceGeneratorAppliedAttribute";

    private const string MarkerSource = """
        // <auto-generated/>
        // Emitted unconditionally by JasperFx.Events.SourceGenerator so the runtime can tell an assembly
        // this generator processed from one it never saw. See jasperfx#887.
        [assembly: global::JasperFx.Events.Aggregation.JasperFxSourceGeneratorApplied]
        """;
